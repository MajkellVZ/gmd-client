...
const ProductImage = ({id}) => {
    ...
    const getMedia = async (id) => {
        const res = await axios.get(`https://gmd-server.xyz/api/media/single_image/product/${id}`);
    ...
    return (
        <div>
            {media ?
                <img className="card-img-top" src={`https://gmd-server.xyz/api/media/${media.image_path}?${Date.now()}`}
